cat << 'INNER_EOF' > snippet_message.txt
...
sed -i -e '/<div className={`space-y-2 max-w-sm pt-4 border-t border-\[#2C2419\]\/50 transition-opacity duration-300 ${whatsappConfig.enableWhatsapp !== false ? '"'"'opacity-100'"'"' : '"'"'opacity-40 pointer-events-none'"'"'}`>}/,/<p className="text-xs text-\[#6B6053\]">Include your country code (e.g., +1 for US, +91 for India, +92 for Pakistan).<\/p>/!b' \
     -e '/<p className="text-xs text-\[#6B6053\]">Include your country code (e.g., +1 for US, +91 for India, +92 for Pakistan).<\/p>/!d; /<p className="text-xs text-\[#6B6053\]">Include your country code (e.g., +1 for US, +91 for India, +92 for Pakistan).<\/p>/{r snippet_message.txt' -e 'd}' app/page.tsx
